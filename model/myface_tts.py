...
from model.comodiffusion import ComoDiffusion
...
class MyFaceTTS(pl.LightningModule):
    def __init__(self, _config,teacher=False,freeze_encoder=False):
        super().__init__()
    ...
    def load_model(self,resume_from):
        self.load_state_dict(torch.load(resume_from,map_location='cuda:0')['state_dict'])
...
# mu_x:(8,128,195), logw:(8,1,195), x_mask:(8,1,195)
        else:
            mu_x, logw, x_mask = self.encoder(x, x_lengths, spk)
            y_max_length = y.shape[-1]
            y_mask = sequence_mask(y_lengths, y_max_length).unsqueeze(1).to(x_mask)
        ...
        mu_y = torch.matmul(attn.squeeze(1).transpose(1, 2), mu_x.transpose(1, 2)) #attn : (8,195,128),mu_x:(8,128,195)
        mu_y = mu_y.transpose(1, 2)#(8,128,128)
        ...
        spk_loss = 0.0
        out = self.syncnet.forward_perceptual(xt_hat.unsqueeze(1)) #length가 8(layer)인 list : (8,96,128,128)
        gt_out = self.syncnet.forward_perceptual(y.unsqueeze(1)) #length가 8인 list : (8,96,128,128)
